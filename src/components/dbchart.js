...
class DashboardColumnChart extends Component {
  ...
  render() {
    const store = this.props.store.model;
    /* access the observables in the model store */
    const data_model = store.dataModel;
    const model_data = store.dailyRisk.map(x => x + 1.0);
    const model_dates = store.modelDates;
    const season = store.seasonDates;

    let start_date = season.seasonStart;
    console.log('start_date : ' + start_date);
    if (model_dates.firstValid && (model_dates.firstValid > start_date)) {
        start_date = model_dates.firstValid;
    }

    const dbchart = data_model.dbchart;
    ...
    var chartConfig = {
        chart: { alignTicks:false, height:300 },
        ...
        rangeSelector: { selected:0, buttonSpacing:6,
            buttons: [ { type:'day', count:10, text:'10 days' },
